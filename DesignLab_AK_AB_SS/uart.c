...
void uart_init(void) {
    ...
    // Włącz przerwania odbioru
    UCSR0B |= (1 << RXCIE0);
    
    // Ustaw format ramki: 8 bitów danych, 1 bit stopu, bez parzystości
...
void uart_flush(void) {
    ...
    (void)dummy; // Unikaj ostrzeżenia o nieużywanej zmiennej
}
// Przerwanie odbioru UART - zapisuje dane do bufora cyklicznego
// UWAGA: ATmega328PB używa USART0_RX_vect (nie USART_RX_vect)
ISR(USART0_RX_vect) {
    uint8_t data = UDR0;
    uint8_t next_head = (rx_head + 1) % UART_RX_BUFFER_SIZE;
    
    // Jeśli bufor nie jest pełny, zapisz dane
    if (next_head != rx_tail) {
        rx_buffer[rx_head] = data;
        rx_head = next_head;
    }
    // W przeciwnym razie dane są tracone (overflow)
}
// Funkcja do odczytu z bufora cyklicznego (nieblokująca)
uint8_t uart_read(void) {
    // Wyłącz przerwania podczas czytania z bufora
    uint8_t sreg = SREG;
    cli();
    
    if (rx_head == rx_tail) {
        SREG = sreg; // Przywróć stan przerwań
